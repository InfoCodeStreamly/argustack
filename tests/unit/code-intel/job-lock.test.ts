...
describe('withJobLock', () => {
  it('acquires + releases lock around fn', async () => {
    ...
    const result = await withJobLock('p1', meta, () => Promise.resolve(42));
  ...
  it('throws if lock cannot be acquired', async () => {
    const meta = new FakeCodeMetaStore();
    meta.locks.add('p1');
    await expect(withJobLock('p1', meta, () => Promise.resolve(0))).rejects.toThrow(/Another indexing job/);
  });

  it('releases lock on error', async () => {
    const meta = new FakeCodeMetaStore();
    await expect(
      withJobLock('p1', meta, () => Promise.reject(new Error('boom'))),
