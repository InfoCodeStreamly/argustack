...
describe('EmbedUseCase', () => {
  ...
  it('embeds issues with summary and description', async () => {
    ...
    expect(storage.hasEmbedding(WS, TEST_IDS.issueKey)).toBe(true);
    expect(storage.hasEmbedding(WS, TEST_IDS.issueKey2)).toBe(true);
  });
  ...
  it('processes in batches', async () => {
    ...
    const result = await useCase.execute(WS, { batchSize: 2 });
  ...
  it('batchSize of 1 processes each issue in a separate embed call', async () => {
    ...
    const result = await useCase.execute(WS, { batchSize: 1 });
  ...
  it('progress message includes batch size count', async () => {
    ...
    const messages: string[] = [];
    await useCase.execute(WS, { batchSize: 2, onProgress: (msg: string) => messages.push(msg) });
  ...
  it('progress reports embedded count after each batch', async () => {
    ...
    const messages: string[] = [];
    await useCase.execute(WS, { batchSize: 1, onProgress: (msg: string) => messages.push(msg) });
