...
```
src/
├── core/types/        ← Domain types (Issue, PullRequest, Commit, Config, CodeProject, CodeSymbol, ...) — zero dependencies
├── core/ports/        ← Interfaces (ISourceProvider, IGitProvider, IGitHubProvider, IDbProvider, IStorage, ICodeGraph, ICodeVectorStore, ICodeParser, ICodeMetaStore, ICodeEmbedding, ILspClient) — contracts only
├── adapters/          ← Driven adapters (jira/, jira-proxy/, git/, github/, csv/, db/, board/, postgres/, openai/, neo4j/, qdrant/, tree-sitter/, lsp/, voyage/, lmstudio/)
├── use-cases/         ← Business logic (pull.ts, pull-git.ts, pull-github.ts, pull-db.ts, push.ts, embed.ts, build-graph.ts, sync-board.ts, move-task.ts, register-code-project.ts, unregister-code-project.ts, index-code.ts, watch-code.ts, code-search.ts)
├── code-intel/        ← Code intelligence orchestrator (indexer.ts, watcher.ts, resolver.ts, lsp-resolver.ts, ranker.ts, chunker.ts, file-discovery.ts, layer-detector.ts, hash.ts, tsconfig-paths.ts, job-lock.ts)
...
│   ├── helpers.ts     ← Shared utilities (loadWorkspace, createCodeAdapters, setActiveCodeAdapters, textResponse)
...
**Code Intelligence (metadata only — graph in Neo4j, vectors in Qdrant):** code_projects, code_files, code_index_jobs

### Code Intelligence Stack (separate Docker hub)

Lives in `~/.argustack/code/docker-compose.yml` (auto-generated by `argustack code init`):
- **Neo4j 5 Community** — code graph (files, symbols, CALLS, IMPORTS, IMPLEMENTS, EXTENDS, INJECTS), ports 7474+7687
- **Qdrant** — vector store, one collection per project (`code_<projectId>`), ports 6333+6334
- **LM Studio** (user-managed, not in compose) — local embedding model on `http://localhost:1234/v1/embeddings`, default Qwen3-Embedding-4B GGUF Q8_0 (license Apache-2.0)

Optional rerank model: set `RERANK_MODEL=<lm-studio-model-id>` env to enable chat-completions-based relevance scoring; without it the rerank stage is a pass-through.
...
| `src/cli/code.ts` | argustack code (init/register/index/watch/list/status/stats/unregister) |
| `src/workspace/registry.ts` | Global workspace registry (~/.argustack/workspaces.json) |
| `src/mcp/server.ts` | MCP server orchestrator — registers 46 tools |
| `src/mcp/tools/` | Tool modules (workspace, query, issue, search, estimate, database, push, graph, code-graph, code-search, code-hybrid, formatters) |
| `src/code-intel/` | Code intelligence orchestrator — indexer, watcher, resolver, LSP resolver, ranker, chunker |
| `src/adapters/neo4j/` | Neo4j adapter (driver, cypher SSOT, graph-store, mapper) |
| `src/adapters/qdrant/` | Qdrant adapter (client, vector-store, mapper) |
| `src/adapters/tree-sitter/` | tree-sitter TypeScript/TSX parser |
| `src/adapters/lsp/` | typescript-language-server client (jsonrpc framing + LSP requests) |
| `src/adapters/lmstudio/` | LM Studio embedding + rerank provider (OpenAI-compat /v1/embeddings + /v1/chat/completions) |
| `src/adapters/voyage/` | Voyage AI embedding + rerank provider (cloud, alternative to LM Studio) |
| `templates/code-docker-compose.yml` | Neo4j + Qdrant compose template (copied to ~/.argustack/code/) |
| `scripts/bench-save-to-searchable.ts` | Save-to-Qdrant latency benchmark (writes test file, polls Qdrant, reports p50/p95) |
